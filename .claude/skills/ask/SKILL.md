...
「典型問題を見て **解答方針** を答える」形式（コードは書かない）。

- 該当 typical90 本問の問題文 / 制約 / サンプル を提示する
- ユーザには **方針だけ** を答えてもらう
  - 使う典型解法（例: 「累積和」「二分探索」「桁DP」）
  - 解法のアイデアの要点 1-3 行
  - 計算量
- ユーザ回答後、模範方針と照合し、合致点 / 抜け / 計算量の妥当性を 3-5 行で指摘
- 「実装まではしない、方針が立てば card は ○」を判定基準とする
