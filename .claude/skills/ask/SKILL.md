...
- 結果確定後 `/log <技法名> impl <o|x> [--uri ...] [--note ...]` を案内（実問題流用時は `--uri` で記録）
...
- 問題URLがあれば示し、`/log <技法名> advanced <o|x> --uri <URL>` を案内
...
- 技法名にスペースや記号が含まれる場合は `/log` 引数で quote する（例: `/log "GCD / LCM" card o`）
